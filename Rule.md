...
#### 転売アクション（2AP）
- **他プレイヤー商品転売**：他プレイヤーのパーソナル・マーケットから商品ダイスを購入し、自分のマーケットに配置する
  - **配置ルール**：コスト値表示はそのままにして、人気度1の価格に対応する座標に配置
  - **1マス1商品制限**：同じ座標には1つの商品のみ配置可能
  - **転売価格（上限20）**：購入価格 + 履歴ボーナス
  - **0-1回**：ボーナスなし（+0資金）
  - **2-4回**：+3資金
  - **5-7回**：+6資金
  - **8回以上**：+10資金
...
### 転売成功時の特殊効果
- **市場汚染**：転売された時点でその商品カテゴリーに汚染マーカー配置
- **威厳減少**：転売を行ったプレイヤーの威厳-1
- **転売履歴記録**：転売を行った時点で転売履歴トラック+1（転売で商品をマーケットに配置してから転売履歴トラック移動）
...
#### 具体例
**フィギュア（製造コスト3、威厳5での価格上限9）の場合**：
- 汚染0個 → 上限9で正規販売可能
- 汚染1個 → 上限8で正規販売（-1ペナルティ）
- 汚染2個 → 上限7で正規販売（-2ペナルティ）
- 汚染3個 → 上限6で正規販売（-3ペナルティ）
- 汚染4個 → 上限5で正規販売（-4ペナルティ）
- 汚染5個以上 → 正規販売不可（転売のみ可能）
